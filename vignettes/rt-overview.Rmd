...
Every time you use `rt`, you'll need to log into your RT installation with `rt_login()`.
Before you can log in, you'll also need to set the base URL of your RT installation.
Every function in `rt` can be passed the base URL via a common `base_url` argument but it's easier to set it globally for your session using `Sys.setenv(RT_BASE_URL="{YOUR_RT_BASE_URL})`.
...
rt_login("guest", "guest")
```

Note: Best Practical (who makes RT) provide a demonstration RT installation that we'll be using for this vignette but in all likelihood you'll be using different values in your scripts.

If you see that logging in was successful, you're ready to use `rt`!

This vignette covers some of the most common functionality:

- [Creating and replying/comment on tickets](#creating-and-interacting-with-tickets)
- [Searching tickets](#searching-tickets)
- [Getting user information](#creating-and-editing-users)

## Creating and interacting with tickets
